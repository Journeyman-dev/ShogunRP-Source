...
after_mission_start_setup = (ti_after_mission_start, 0, 0, [], # spawn and move certain things after most other set up is done
   [(set_spawn_effector_scene_prop_kind, 0, -1),
	...
	(call_script, "script_setup_ice_balls"), #Ramaraunt add
    (init_position, pos1),
...
agent_killed = (ti_on_agent_killed_or_wounded, 0, 0, [], # server and clients: handle messages, score, loot, and more after agents die
   [(store_trigger_param_1, ":dead_agent_id"),
	...
	(try_begin),
		...
		(call_script, "script_necromancer_kill_own_undead", ":dead_agent_id"), #RAMARAUNT ADD
		(agent_get_player_id, ":dead_player_id", ":dead_agent_id"),
		(call_script, "script_plague_death_calculate", ":dead_player_id"),
		(player_slot_eq,":dead_player_id", slot_player_zombie_infected,1),
		(try_begin),
				(player_get_slot, ":necromancer_player_id", ":dead_player_id", slot_player_zombie_necromancer),
				(neq, -1, ":necromancer_player_id"),
				(call_script, "script_transform_player_into_zombie", ":dead_agent_id", 1, ":necromancer_player_id", 0),
		(else_try),
				(agent_get_player_id, ":killer_player_id", ":killer_agent_id"),
				(call_script, "script_transform_player_into_zombie", ":dead_agent_id", 1, ":killer_player_id", 0),
		(try_end),
		(call_script, "script_reset_illnesses", ":dead_player_id"), #RAMARAUNT ADD
	(try_end),

	(try_begin),
		(agent_get_troop_id, ":dead_troop_id", ":dead_agent_id"),
		(this_or_next|eq, ":dead_troop_id", "trp_zombie"),
		(this_or_next|eq, ":dead_troop_id", "trp_fast_zombie"),
		(this_or_next|eq, ":dead_troop_id", "trp_rotting_zombie"),
		(this_or_next|eq, ":dead_troop_id", "trp_tank_zombie"),
		(this_or_next|eq, ":dead_troop_id", "trp_skeleton_archer"),
		(eq, ":dead_troop_id", "trp_skeleton_berserker"),
		(val_sub, "$g_zombie_count", 1),
	(else_try),
		(eq, ":dead_troop_id", "trp_rock_golem"),
		(val_sub, "$g_golem_count", 1),
	(try_end),
...
taxation_money_add = (30, 0, 0, [], #Ramaraunt add extra castle tax money
	[(multiplayer_is_server),
	(call_script,"script_castle_recieve_taxes"),
	])
...
def common_triggers(self):
  return [(ti_before_mission_start, 0, 0, [(assign, "$g_game_type", "mt_" + self)], []),
	...
	taxation_money_add, #Ramaraunt add
